...
var _TOUR_STEPS = [
  {
    ...
    body: '7,000+ medical cards with SM-2 spaced repetition. Each card returns exactly when your brain is about to forget it — smarter than re-reading notes.',
    pos: 'bottom',
  },
  {
  ...
  },
  {
    ...
    body: 'Timed multiple choice that mirrors real exam conditions. Accuracy is tracked per category so you know exactly where to focus.',
    pos: 'bottom',
  },
  {
    id: 'hosa', target: '.mc--hosa', emoji: '🏆', color: '#d97706',
    title: 'Competitive Events',
    body: 'Every event, study guide, scoring rubric, and practice resource — organised by event and always up to date. Everything in one place.',
    pos: 'bottom',
  },
  {
    id: 'progress', target: '.mc--progress', emoji: '📈', color: '#059669',
    title: 'Track your growth',
    body: 'XP, streaks, mastery ratings, and weak spots. See exactly what you\'ve learned and what needs another pass — nothing slips through.',
    pos: 'auto',
  },
  {
    id: 'club', target: '.mc--club', emoji: '🏫', color: '#0d9488',
    title: 'Club Dashboard',
    body: 'Create or join a study group. Assign targeted practice, track the whole class, and see exactly who needs help with what.',
    pos: 'top',
  },
];
...
function tourStart(profile) {
  ...
  setTimeout(function() {
    ...
    _tourBuildDOM();
    setTimeout(function() { _tourRender(0); }, 240);
  }, 800);
}
...
function _tourBuildDOM() {
  ...
  r.innerHTML =
   ...
   +    '<div class="t-finish-p">Everything you need to learn healthcare is right here.<br>Build your first streak today.</div>'
...
function _movePanels(rect, pad) {
  ...
  var t = Math.max(0,rect.top-pad), b = Math.max(0,H-rect.bottom-pad);
  var l = Math.max(0,rect.left-pad), rr = Math.max(0,W-rect.right-pad);
  ...
  tp.style.height=t+'px'; bp.style.height=b+'px';
...
function _moveRing(rect, pad, color) {
  ...
  rg.style.borderColor=color; rg.style.color=color;
  rg.style.boxShadow='0 0 0 4px '+color+'22, 0 0 32px '+color+'38';
  rg.style.opacity='1';
}

/* ── Tooltip content ──────────────────── */
function _fillTooltip(s, rect, pad, idx) {
  var total=_t.steps.length, tt=document.getElementById('t-tip'),
      next=document.getElementById('t-next'), back=document.getElementById('t-back'),
      em=document.getElementById('t-emoji');
  var dots='';
  for (var i=0;i<total;i++) dots+='<div class="t-dot '+(i<idx?'done':i===idx?'active':'')+'"></div>';
...
function _placeTooltip(rect, pos, pad) {
  var tt=document.getElementById('t-tip'), W=window.innerWidth, H=window.innerHeight;
  var ttW=W<520?W-32:306, ttH=tt.offsetHeight||230, mar=16;
...
function _tourShowFinish() {
  ...
  setTimeout(_launchConfetti, 380);
}
...
function _launchConfetti() {
  ...
  var html='';
  for (var i=0;i<64;i++) {
    var c=colors[i%colors.length], dx=(Math.random()*320-160).toFixed(1);
    var dy=-(Math.random()*260+80).toFixed(1), rot=(Math.random()*720-360).toFixed(1);
    var del=(Math.random()*0.4).toFixed(2), w=(Math.random()*11+4).toFixed(1);
...
function _tourOnResize() {
  ...
  _tourResizeTimer=setTimeout(function() {
    ...
    _movePanels(rect,12); _moveRing(rect,12,s.color); _placeTooltip(rect,s.pos,12);
  },80);
}
